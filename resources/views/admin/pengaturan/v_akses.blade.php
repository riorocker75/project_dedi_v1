          ...
          <div class="row">
            <section class="col-lg-5 connectedSortable">
              <div class="card">
                ...
                <div class="card-body">
                <form action="{{url('/admin/pengaturan/akses/tambah')}}" method="post">
                    @csrf
                    @php
                        $peg=App\Model\Operator::whereIn('jabatan',[1,2,3])->orderBy('operator_id','asc')->get();
                    @endphp
                    <div class="form-group">
                      <label for="">Cari Nama Pegawai</label>
                      <select class="selectpicker form-control form-control-user" name="pegawai" data-live-search="true" required>
                        <option value="" selected>--Pilih--</option>
                        
                        <?php foreach ($peg as $pd){?>
                            <option value="<?php echo $pd->operator_kode?>" data-tokens="<?php echo $pd->operator_nomor_pegawai?> | <?php echo $pd->operator_nama?>"> <?php echo $pd->operator_nama?> | <?php echo jabatan($pd->jabatan)?></option>
                          <?php  } ?>
                    </select>
                    </div>
                    <div class="form-group">
